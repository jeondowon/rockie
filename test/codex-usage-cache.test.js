...
test("가장 최근 세션 로그의 마지막 한도 스냅샷을 쓴다", async () => {
  ...
  assert.equal(usage.limit.usedPercent, 12); // 마지막 줄의 값
  assert.equal(usage.limit.planType, "pro");
  assert.equal(usage.limit.resetsAt, RESETS_AT);
});
...
test("끊긴 마지막 줄은 다음 갱신까지 미룬다", async () => {
  ...
  assert.equal(usage.limit.usedPercent, 11); // 깨진 줄은 아직 읽지 않는다
...
test("파일이 잘리거나 교체되면 처음부터 다시 읽는다", async () => {
  ...
  const cache = new CodexUsageCache(dir);
  assert.equal(
    (await cache.reload({ keepPreviousOnFailure: false })).limit.usedPercent,
    12,
  );
...
test("이벤트 원본을 메모리에 쌓아두지 않는다", async () => {
  ...
  const cache = new CodexUsageCache(dir);
  const usage = await cache.reload({ keepPreviousOnFailure: false });
  assert.equal(usage.limit.usedPercent, 20);

  assert.equal(cache.file.events, undefined); // 이벤트 배열 자체가 없어야 한다
  assert.equal(cache.file.limit.usedPercent, 20); // 남는 건 최신 스냅샷 1개뿐
});
...
test("새 세션 파일이 생기면 그쪽 스냅샷으로 갈아탄다", async () => {
  ...
  const usage = await cache.reload();
  assert.equal(usage.limit.usedPercent, 42);
...
test("한도 정보 없는 이벤트가 마지막이어도 직전 스냅샷을 유지한다", async () => {
  ...
  assert.equal(usage.limit.usedPercent, 37);
});
