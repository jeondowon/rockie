...
const {
  ...
  MAIN_QUESTIONS,
  EI_QUESTIONS,
} = require("../src/main/questions");

const mainIds = () => MAIN_QUESTIONS.map((q) => q.id);
const eiIds = () => EI_QUESTIONS.map((q) => q.id);
...
function makeData({ onboardingCompleted = true } = {}) {
  return {
    ...
    onboarding: {
      ...
      completedAt: onboardingCompleted ? "2026-07-09T00:00:00.000Z" : null,
    },
    pet: {
      petName: null,
      petNameSetAt: null,
...
function answerOnboardingIds(data, ids, value) {
  let result = null;
  ids.forEach((id, index) => {
    result = evolution.answerOnboarding(data, {
      questionId: id,
      value,
      nextStep: index + 1,
    });
...
test("본 질문 완료 후 원석 1단계로 확정된다", () => {
  ...
  assert.equal(data.questions.mainQuestionProgress, MAIN_QUESTIONS.length);
...
test("본 질문 2개 돌 동점이면 해당 타이브레이커를 우선 삽입하고 답변 후 확정한다", () => {
  ...
  answerIds(data, ids.slice(0, ids.length / 2), "granite");
  const resultAfterTie = answerIds(data, ids.slice(ids.length / 2), "basalt");
...
test("1단계에서 E/I 질문 완료 후 변성체 2단계로 확정된다", () => {
  const data = makeData();
...
test("E/I 6:6 동점이면 타이브레이커를 우선 삽입하고 답변 후 확정한다", () => {
  ...
  answerIds(data, ids.slice(0, 3), "외향");
  const resultAfterTie = answerIds(data, ids.slice(3), "내향");
...
test("성향 태그는 카테고리별 최다 답변 태그로 계산된다", () => {
  ...
  answerIds(data, ["main_01", "main_06"], "marble");
  answerIds(data, ["main_02"], "gneiss");

  const state = evolution.getState(data);

  assert.deepEqual(state.tags.slice(0, 2), ["동행 조율형", "맥락 파악형"]);
});
...
test("온보딩 질문은 0→1 점수에 반영되고 완료 후 오늘 질문 3개를 채운다", () => {
  ...
  const completed = evolution.completeOnboarding(data);
