...
- 전체 npm 패키지 라이선스 목록 생성
- `Third-party notices` 문서 작성
- 앱 또는 웹사이트에서 오픈소스 라이선스 접근 가능하게 하기
...
필요하면 별도 라이선스 추출 도구를 도입한다.

### 5.2 폰트/이미지/GIF

확인 대상:

- `assets/fonts/Galmuri*.woff2`
- `assets/gif/`
- `assets/img/`
- `assets/tray/`
...
- 앱 패키징 방식 결정
- macOS 실행 파일 생성
- 화면 기록 권한 없이도 기본 기능이 정상 동작하는지 확인
- 민감 화면에서 자동 말풍선이 뜨지 않는지 확인
- `npm test` 통과
- 실제 빌드 파일을 다운로드한 것처럼 실행 테스트
...
- `privacy-policy.md`: 개인정보처리방침 초안
- `terms.md`: 이용약관 초안
- `third-party-notices.md`: 오픈소스 라이선스 고지
- `install.md`: 사용자용 설치/권한 안내
- `release-notes.md`: 버전별 변경사항
