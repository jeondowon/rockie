...
const CHAR_SIZE = 128; // 실제 렌더링 크기(style.css의 .character width/height)와 일치시켜야 좌표 계산이 정확함
...
// (표시 크기 128px 기준 측정값 ≈ 36px, 0으로 두면 박스 경계 기준으로 돌아감)
const SPRITE_MARGIN = 36;
...
function groundY() {
  return window.innerHeight - 100; // Dock이 없을 때 캐릭터가 서는 기본 라인
}
...
function followStep() {
  if (!paused && !cardOpen) {
    ...
    else if (delta < -MAX_SPEED) delta = -MAX_SPEED;

    posX = clampX(posX + delta);
...
function positionBubble() {
  // 캐릭터 그림의 실제 가로 중심(여백이 좌우 대칭이라 박스 중심과 같음)에
  // 말풍선을 가운데 정렬한다. 화면 밖으로 나가지 않도록 클램프.
  const centerX = posX + CHAR_SIZE / 2;
  const bubbleWidth = bubble.offsetWidth;
  const left = Math.max(
    0,
    Math.min(window.innerWidth - bubbleWidth, centerX - bubbleWidth / 2)
  );
